...
Returns `StepResponse` with initial observation and `episode_id` in `info`.
...
Pass the `episode_id` from `/reset` as the `X-Episode-Id` header. Returns `StepResponse` with reward and done flag.

### `GET /state`

Pass `X-Episode-Id` header. Returns current `TicketState`.

### `GET /tasks`

Returns metadata for all three tasks (no header required).
